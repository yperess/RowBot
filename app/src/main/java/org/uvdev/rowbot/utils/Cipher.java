...
public class Cipher {
    ...
    private static final String GITHUB_SALT = "a8h+YzEGTRNOCnBmnl1I8XYuhAlEMxFNSVqFIYNWhq0=";
    private static final String GITHUB_IV = "19FdvE6sJaok+EY092rdrA==";
    private static final String GITHUB_ACCESS_TOKEN =
            "9SUtpl0doSqsVBN+w4+K9pxwt27bfsIGt7UO53Z6yQz1K6HMHg0lAx0WJbx/l33u";
    ...
    public static void encrypt(String password, String data)
        ...
        // TODO consider returning these in a new class.
    }
    ...
    public static String getGithubAccessToken(String password) {
        ...
        try {
            KeySpec keySpec = new PBEKeySpec(password.toCharArray(),
                    Base64.decode(GITHUB_SALT, Base64.DEFAULT), ITERATION_COUNT, KEY_LENGTH);
            SecretKeyFactory keyFactory = SecretKeyFactory.getInstance("PBKDF2WithHmacSHA1");
            byte[] keyBytes = keyFactory.generateSecret(keySpec).getEncoded();
            SecretKey key = new SecretKeySpec(keyBytes, "AES");

            javax.crypto.Cipher cipher = javax.crypto.Cipher.getInstance("AES/CBC/PKCS5Padding");
            IvParameterSpec ivParams = new IvParameterSpec(Base64.decode(GITHUB_IV, Base64.DEFAULT));
            cipher.init(javax.crypto.Cipher.DECRYPT_MODE, key, ivParams);
            token = new String(cipher.doFinal(Base64.decode(GITHUB_ACCESS_TOKEN, Base64.DEFAULT)),
                    "UTF-8");
